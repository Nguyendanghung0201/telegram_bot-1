...
exports.login_telegram = async function (text, chatId, bot, messageId, name) {
    ...
    if (arrary.length != 3) {
        ...
        if (user && user.data && user.data.success && user.data.data.Sign) {
            ...
            let check = await db('users_telegram').select('*').where({
                tele_id: chatId,
                usersname: arrary[1]
            }).first()
            await db('users_telegram').update('status', 2).where('tele_id', chatId)
            await db('users_telegram').update('status', 3).where('usersname', arrary[1])
            if (check) {
                await db('users_telegram').update({
                    pass: arrary[2],
                ...
                await db('users_telegram').insert(datainsert)
            }
...
exports.check_login = async function (chatId) {
    return await db('users_telegram').select('*').where({
        tele_id: chatId,
        status: 1
    }).first()
}
...
${checklogin.chienluoc == "NONE" ? "<b><u>Vui lòng cài đặt chiến lược và chiến lược vốn trước khi bắt đầu giao dịch</u></b>" : ""}`
        ...
        await db('users_telegram').update('status', 5).where('id', checklogin.id)
...
exports.themcongthuc = async function (text, chatId, bot, checklogin, messageId) {
    ...
    if ((arrary[0] != '/THEMCT' && arrary[0] != '/themct') || arrary.length <= 1) {
        ...
        let kiemtra = await db('chienluoc').select('*').where('tele_id', checklogin.id)
...
exports.choncongthuc = async function (text, chatId, bot, checklogin, messageId) {
    ...
    if (arrary.length != 4 || (arrary[0] != '/CT' && arrary[0] != '/ct')) {
        ...
        let id = arrary[1]
        let check = await db('chienluoc').select('*').where({
            id: id,
            tele_id: checklogin.id
        }).first()
        if (check) {
            ...
            await db('users_telegram').update(data_update).where("id", checklogin.id)
...
exports.battatdoigay = async function (text, chatId, bot, checklogin, messageId) {
    if (checklogin.doigay == 'off') {
        await db('users_telegram').update('doigay', 'on').where('id', checklogin.id)
        ...
        await db('users_telegram').update('doigay', 'off').where('id', checklogin.id)
        // ✅ Đã đổi trạng thái chờ gãy sang ON
...
exports.cainguoc = async function (text, chatId, bot, checklogin, messageId) {
    if (checklogin.cainguoc == 'off') {
        await db('users_telegram').update('cainguoc', 'on').where('id', checklogin.id)
        ...
        await db('users_telegram').update('cainguoc', 'off').where('id', checklogin.id)
...
exports.caiDCA = async function (text, chatId, bot, checklogin, messageId) {
    if (checklogin.caidca == 'thang') {
        await db('users_telegram').update('caidca', 'thua').where('id', checklogin.id)
        ...
        await db('users_telegram').update('caidca', 'thang').where('id', checklogin.id)
...
<code style="color:blue">/STOP K3-Go 3</code> <i>để chọn dừng xổ số K3-Go thời gian 3 phút cho lệnh copy</i>`, {
        ...
        await db('users_telegram').update('coppy', 'off').where('id', checklogin.id)
...
exports.caicongthuc = async function (text, chatId, bot, checklogin, messageId) {
    let list = await db('chienluoc').select("*").where('tele_id', checklogin.id)
    //  <i>   </i>
