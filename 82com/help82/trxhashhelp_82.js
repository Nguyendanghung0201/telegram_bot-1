...
exports.login_telegram = async function (text, chatId, bot, messageId, name,userLink) {
    ...
    if (arrary.length != 3) {
        ...
        let user = await axios({
            method: 'post',
            url: 'https://82vn82vnapi.com/api/webapi/UserLogin',
            data: {
                username: arrary[1],
                pwd: arrary[2],
                regtype: "1",
...
exports.batdau = async function (text, chatId, bot, checklogin, messageId) {
    let amount = await axios.post("https://82vn82vnapi.com/api/webapi/GetUserInfo", {
    ...
    if (amount && amount.data && amount.data.data && amount.data.data.Amount) {
        ...
        let text = `Chào mừng <b>${checklogin.tele_name}</b> đến với Auto 82VN!
