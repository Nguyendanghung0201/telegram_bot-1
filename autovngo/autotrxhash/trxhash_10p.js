...
async function tonghopphien(data_copy, gay, tim_kiem, tinhieu, bot) {
    try {
        ...
        .where('group_id', data_copy.id_group).andWhere("type", 'trxhash10')
        .orderBy('id', 'desc')
    ...
    try {
        ...
        for (let item of list_thang_da_chon) {
            ...
            if (gan_nhat && gan_nhat.length > 0) {
                ...
                let data_copy = await db("copytinhieu_trxhash").select("*").where('start', 1).andWhere("type", "10").andWhere("id_group", item.group_id).first()
                if (!data_copy) {
                    continue
                }
        ...
        let list = await db("copytinhieu_trxhash").select("*").where('start', 1).andWhere("type", "10")
    ...
    try {
      ...
      let list_lich_su = await axios.post("https://bdguubdg.com/api/webapi/GetTRXNoaverageEmerdList", {
        ...
        if (list_lich_su.data && list_lich_su.data.data && list_lich_su.data.success) {
            ...
            let trybonhotam = await db('bonhotam').select('*').where('issuenumber', issuenumber).andWhere('type', 'trxhash10').andWhere("status", 1).first()
            if (trybonhotam) {
                return
            }
            ...
            let data_copy = await db('copytinhieu_trxhash').select('*').where('status', 1).andWhere("type", "10").first()
            if (data_copy) {
        ...
        if (result.data) {
            ...
            if (result.data && result.data.data && result.data.code == 0 && result.data.success) {
                ...
                if (msg == "Số tiền không đủ") {
                    await db(table).update('trxhash10', 0).where('id', item.id)
...
Tổng lợi nhuận: ${data_loi_nhuan[element.usersname]}đ`)
    ...
    await db("bonhotam").update('status', 0).where('issuenumber', item.IssueNumber).andWhere('type', 'trxhash10').andWhere("status", 1)
}
async function xacdinhlichsu(gameslist, bot) {
    ...
    for (let item of gameslist) {
        ...
        if (!bonhotam[item.IssueNumber]) {
            let trybonhotam = await db('bonhotam').select('*').where('issuenumber', item.IssueNumber).andWhere('type', 'trxhash10').andWhere("status", 1).first()
            if (trybonhotam) {
