...
async function guigaytoicacuser(len, bot) {
    let list = await db(table).select("*").where('doigay', 'on').andWhere('k3go3', 1)
    await db(table).update('doigay', 'off').where('doigay', 'on').andWhere('k3go3', 1)
    for (let el of list) {
        bot.sendMessage(el.chatId, `🔂 Tín hiệu đã gãy K3-GO 3 phút, bắt đầu copy tín hiệu
Entry: 0
Len: ${len}`)
        await delay(300)
    }
...
async function tonghopphien(data_copy, gay, tim_kiem, tinhieu, bot) {
    try {
        ...
        .where('group_id', data_copy.id_group).andWhere("type", 'k3go3')
        .orderBy('id', 'desc')
...
async function test(bot) {
    ...
    try {
        ...
        if (data.data && data.data.success) {
            ...
            let data_1phut = datalist.filter(e => e.Type == 10)[0]
    ...
    try {
        ...
        for (let item of list_thang_da_chon) {
            ...
            if (gan_nhat && gan_nhat.length > 0) {
                ...
                let data_copy = await db("copytinhieu_k3go").select("*").where('start', 1).andWhere("type", "3").andWhere("id_group", item.group_id).first()
                if (!data_copy) {
                    continue
                }
        ...
        let list = await db("copytinhieu_k3go").select("*").where('start', 1).andWhere("type", "3")
...
async function check_dk(issuenumber, bot) {
    ...
    try {
        ...
        if (list_lich_su.data && list_lich_su.data.data && list_lich_su.data.success) {
            ...
            let trybonhotam = await db('bonhotam').select('*').where('issuenumber', issuenumber).andWhere('type', 'k3go3').andWhere("status", 1).first()
            if (trybonhotam) {
                return
            }
            ...
            let data_copy = await db('copytinhieu_k3go').select('*').where('status', 1).andWhere("type", "3").first()
            if (data_copy) {
    ...
    try {
        ...
        if (result.data) {
            ...
            if (result.data && result.data.data && result.data.code == 0 && result.data.success) {
                ...
                bot.sendMessage(item.tele_id, `✅ Đã đặt cược K3-GO 3 ${data.selecttype == "H" ? "Lớn" : "Nhỏ"} - ${data.betcount}000đ - Kỳ xổ ${issuenumber}`,)
            } else {
                //  đặt cược lỗi
                let msg = result.data.msg
                if (msg == "Số tiền không đủ") {
                    await db(table).update('k3go3', 0).where('id', item.id)
...
async function ketqua_run_bot(ketqua, item, bot, Number_one) {
    for (let element of bonhotam[item.IssueNumber]) {
        ...
        if (element.selecttype == ketqua) {
            ...
            bot.sendMessage(element.chatId, `🟢 Chúc mừng bạn đã thắng ${Math.round(parseInt(element.betcount) * 0.96 * 1000)}đ K3-GO 3 kì ${element.issuenumber}
Tổng lợi nhuận: ${data_loi_nhuan[element.usersname]}đ`)
            ...
            if (element.loidung) {
                ...
                if (data_loi_nhuan[element.usersname] > element.loidung) {
                    ...
                    await db(table).update('k3go3', 0).where('id', element.id)
            ...
            bot.sendMessage(element.chatId, `🔴 Rất tiếc bạn đã thua ${element.betcount}000đ K3-GO 3 kì ${element.issuenumber}`)
            // await db('lichsu_ma').insert({
            //     "uid": element.uid,
            //     "usersid": element.id,
            //     "gametype": element.gametype,
    ...
    await db("bonhotam").update('status', 0).where('issuenumber', item.IssueNumber).andWhere('type', 'k3go3').andWhere("status", 1)
}
async function xacdinhlichsu(gameslist, bot) {
    ...
    for (let item of gameslist) {
        ...
        if (!bonhotam[item.IssueNumber]) {
            let trybonhotam = await db('bonhotam').select('*').where('issuenumber', item.IssueNumber).andWhere('type', 'k3go3').andWhere("status", 1).first()
            if (trybonhotam) {
