...
async function ketqua_run_bot(ketqua, item, bot, Number_one) {
    for (let element of bonhotam[item.IssueNumber]) {
        ...
        if (element.selecttype == ketqua) {
            ...
            bot.sendMessage(element.chatId, `🟢 Chúc mừng bạn đã thắng ${Math.round(parseInt(element.betcount) * 0.96 * 1000)}đ 5D-Go 3 kì ${element.issuenumber}
Tổng lợi nhuận: ${data_loi_nhuan[element.usersname]}đ`)
            ...
            bot.sendMessage(element.chatId, `🔴 Rất tiếc bạn đã thua ${element.betcount}000đ 5D-Go 3 kì ${element.issuenumber}`)
            // await db('lichsu_ma').insert({
            //     "uid": element.uid,
            //     "usersid": element.id,
            //     "gametype": element.gametype,
