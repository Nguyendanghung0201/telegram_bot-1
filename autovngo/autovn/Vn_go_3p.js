...
async function test(bot) {
    try {
        ...
        if (data.data && data.data.success) {
            ...
            let data_1phut = datalist.filter(e => e.Type == 2)[0]
...
Tổng lợi nhuận: ${data_loi_nhuan[element.usersname]}đ`)
            ...
            bot.sendMessage(element.chatId, `🔴 Rất tiếc bạn đã thua ${element.betcount * 1000} Vn-Go 3 kì ${element.issuenumber}`)
            // await db('lichsu_ma').insert({
            //     "uid": element.uid,
            //     "usersid": element.id,
            //     "gametype": element.gametype,
